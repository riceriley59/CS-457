...
const vec3 SHALLOW_WATER = vec3(0.0, 0.5, 0.8);
const vec3 DEEP_WATER = vec3(0.0, 0.2, 0.5);
...
const vec3 LAND_SPECULAR = vec3(1.0, 1.0, 1.0);
const vec3 WATER_SPECULAR = vec3(0.8, 0.9, 1.0);
...
void main() {
    ...
    vec3 specularColor = LAND_SPECULAR;
    ...
    // Reduce specular intensity for water
    if (elevation <= WATER_LEVEL) {
        specular *= 1.5;
    }
